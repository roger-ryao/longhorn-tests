...
resource "time_sleep" "wait_controlplane_3_minute_k3s" {
...
resource "aws_ec2_instance_state" "controlplane_state_k3s" {
  ...
  depends_on = [
    time_sleep.wait_controlplane_3_minute_k3s
  ]
...
##resource "null_resource" "verify_packages_controlplane_k3s" {
##  count = var.k8s_distro_name == "k3s" ? var.lh_aws_instance_count_controlplane : 0
##
##  depends_on = [
...
resource "time_sleep" "wait_worker_3_minute_k3s" {
...
resource "aws_ec2_instance_state" "worker_state_k3s" {
  ...
  depends_on = [
    time_sleep.wait_worker_3_minute_k3s
  ]
...
### node initialization step 2.5: verify required packages are active after reboot
##resource "null_resource" "verify_packages_worker_k3s" {
##  count = var.k8s_distro_name == "k3s" ? var.lh_aws_instance_count_worker : 0
##
##  depends_on = [
...
resource "null_resource" "cluster_setup_controlplane_k3s" {
  ...
  depends_on = [
    aws_ec2_instance_state.controlplane_state_k3s
  #  null_resource.verify_packages_controlplane_k3s,
  ]
...
resource "null_resource" "cluster_setup_worker_k3s" {
  ...
  depends_on = [
    aws_ec2_instance_state.worker_state_k3s,
#    null_resource.verify_packages_worker_k3s,
